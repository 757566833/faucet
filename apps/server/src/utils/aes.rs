...
pub fn encrypt_data(timestamp: u64) -> Result<String, String> {
    ...
    if let (Some(cipher), Some(nonce)) = (cipher_option, nonce_option) {
        ...
        let ciphertext_result = cipher.encrypt(&nonce, binding.as_bytes().as_ref());
        ...
        return Err(String::from("no chiper"));
    }
}
...
pub fn decrypt_data(b64: String) -> Result<u64, String> {
    ...
    let cipher_option = CIPHER_ONCE.get();
    let nonce_option = NONCE_ONCE.get();
    if let (Ok(vec), Some(cipher), Some(nonce)) = (vec_result, cipher_option, nonce_option) {
    ...
    return Err(String::from("cant parse"));
}
