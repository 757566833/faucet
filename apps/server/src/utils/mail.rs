...
use crate::{
    constant::{MAIL_FROM_DOMAIN, MAIL_FROM_NAME, MAIL_FROM_PASSWORD, MAIL_SMTP},
    error::{email_format_error, ResponseError},
};

pub async fn send_email(to: String, context: String) -> Result<bool, ResponseError> {
    let from_user = env::var(MAIL_FROM_NAME)?;
    let from_domain = env::var(MAIL_FROM_DOMAIN)?;
    let from_password = env::var(MAIL_FROM_PASSWORD)?;
    let smtp = env::var(MAIL_SMTP)?;
